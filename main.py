...
app = Client("bsdkbete",api_id=2408773   ,api_hash="e365d94ffd4b402e464f082f0ae43fe5"  ,bot_token="1943454168:AAF_c18wgRjswPpuP6O9Pq9kK_IrnwNgOrY"  )
...
@app.on_message(filters.command(["split","split@DECRYPTOR_ROBOT"]))
async def split(client,message):
 ...
 lines_per_file=int(a)
 smallfile = None
 old_msg=await message.reply("Spliting file "+path+" please wait ")
 allfiles=[]
 with open(path) as bigfile:
    for lineno, line in enumerate(bigfile):
        if lineno % lines_per_file == 0:
            if smallfile:
                smallfile.close()
            small_filename = '{}_{}.csv'.format(path,lineno + lines_per_file)
